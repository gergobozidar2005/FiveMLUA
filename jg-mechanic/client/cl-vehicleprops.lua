---Get statebags from mechanic_vehicledata & set them
---@param vehicle integer
---@param plate string|false
local function retrieveAndApplyVehicleStatebagData(vehicle, plate)
  ...
  -- Set defaultStance & baseHandling just locally; these don't need to be sent to the server cache 
  if not state.defaultStance then state:set("defaultStance", defaultStance, true) end
  if not state.baseHandling then state:set("baseHandling", baseHandling, true) end
...
function setVehicleProperties(vehicle, props, withStatebags)
  ...
  if props.color2 then
    if type(props.color2) == "number" then
      ...
      -- If color2 is numeric (not rgb), it breaks paintType1 if color1 is rgb. so we just need to re set it.
